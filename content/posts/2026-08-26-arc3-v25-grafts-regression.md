...
在 V24 上加 7 個 TAAF grafts (winframe+goalkeep+clockwatch+hudmask+clickmap+searchmap+lawbook)。但過程中掉了 FP8 KV、WBC、RE cap、NG。LB 1.42，比 V24 -1.14。

## Context

V24 用 9 個 patch 拿到 2.56。假設：在 V24 config 上加 7 個 TAAF grafts (winframe、goalkeep、clockwatch、hudmask、clickmap、searchmap、lawbook) 應該複合到 LB 3.0+。
...
預期：這些 grafts 會給 solver 更多資訊，改善 LB。

## 技術選擇

用 `thtennant/taaf-kaggle-source-share-fork` 當 source bundle。7 個 grafts 透過 `TAAF_GRAFTS_FLAGS` env var 啟用。

Graft 安裝已驗證：stdout 顯示 `TAAF_GRAFTS FEATURES={"clickmap":true,...,"winframe":true} API_VERSION=1` 與個別 `[goalkeep] armed`、`[hudmask] armed` 等 line。
...
歸因清楚：是掉掉的 patch 造成退步，不是 grafts。具體來說：
...
教訓：加 feature 不是免費的，如果基礎退步了。V25 加了 7 個 grafts 但掉了 4 個 patch。淨效果是 -1.14 LB。

修法應該是把 7 個 grafts port 到 V24 的 bundle 上，不是 thtennant fork。V28 會用 text-only ablation 替代嘗試。

## 下一版計畫

V26/V27 是 MM-ablation 實驗 (text-only vs image-only)。V28 會在 V24 config 上做 text-only ablation，測 hidden games 是否偏好 image modality。
