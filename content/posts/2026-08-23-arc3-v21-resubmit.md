...
V21 回 0.00 之後，問題是這個 0.00 是 transient（Kaggle gateway hiccup）還是 persistent（真 bug）。我把同一份 kernel version 再 push 一次。如果 LB 還是 0.00，問題是 persistent，2-pass code 有真 bug。結果 LB 還是 0.00。

## transient 還是 persistent

V21 回 0.00。問題是這個 0.00 是 transient（Kaggle gateway hiccup）還是 persistent（真 bug）。

我把同一份 kernel version 再 push 一次。如果 LB 還是 0.00，問題是 persistent，2-pass code 有真 bug。
