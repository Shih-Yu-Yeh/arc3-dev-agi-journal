...
總計 2443 個 `UnboundLocalError` 在 step_env。每個例外都被 try/except 接住，所以 kernel 沒崩潰。但每個 step 都退回預設 solver 行為，繞過 6 個 modules。

這是最陰險的失敗方式：kernel 完成、提交、回真實 LB 分數。沒有 patch 驗證的話，會以為 6 個 modules 造成退步。實際上 6 個 modules 從未執行任何 step。
